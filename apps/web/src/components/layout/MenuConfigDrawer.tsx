...
interface MenuConfigDrawerProps {
  open: boolean;
  onClose: () => void;
}
...
export function MenuConfigDrawer({ open, onClose }: MenuConfigDrawerProps) {
  ...
  useEffect(() => {
    if (open) {
      setLocalTop(stTop.map(i => ({ ...i })));
      setLocalGroups(cloneGroups(stGroups));
      setEditing(null);
    }
  }, [open]);
  ...
  function EditBtn({ itemKey, label }: { itemKey: string; label: string }) {
    ...
    return (
      <Tooltip title="Đổi tên (double-click)">
        <Button
      ...
      {localGroups.map((group, gIdx) => (
        ...
        >
          ...
          {group.items.length === 0 && (
            <div style={{ padding: '8px 18px', fontSize: 12, color: mutedText }}>
              (Không có item)
            </div>
          )}
              ...
              <Space size={2} style={{ marginLeft: 'auto', flexShrink: 0 }}>
                <Tooltip title="Lên">
                  <Button
                    size="small" type="text" icon={<ArrowUpOutlined />}
                    disabled={iIdx === 0}
                    onClick={() => moveItem(group.key, item.key, 'up')}
                  />
                </Tooltip>
                <Tooltip title="Xuống">
                  <Button
                    ...
                    onClick={() => moveItem(group.key, item.key, 'down')}
                  />
                </Tooltip>
                <Select
                  size="small"
